...
<template>
	<div class="dishes">
		<form class="submit-form animated lightSpeedIn">
			<h2>推荐菜单管理</h2>
	...
	export default{
		...
		methods:{
			...
			async updata(){
				...
				let res=await ajax('http://localhost:3000/api/addrecommendmenu',values)
				if(res.code===1){
					this.$message({
						message:'推荐菜品上架成功',
						type:'success'
			...
			async downdata(){
				...
				let res=await ajax('http://localhost:3000/api/deleterecommendmenu',values)
				if(res.code===1){
					this.$message({
						message:'推荐菜品下架成功',
						type:'success'
